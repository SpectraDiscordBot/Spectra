...
class ManageRoles(commands.Cog):
    ...
    async def create(
        self, ctx: commands.Context, name: str, color: str, mentionable: bool = False
    ):
        try:
            try:
            ...
            await ctx.send(
                f"<:Checkmark:1326642406086410317> Created role {role.mention}."
            )
    ...
    async def delete(self, ctx: commands.Context, role: discord.Role):
        try:
            ...
            await ctx.send(
                f"<:Checkmark:1326642406086410317> Deleted role {role.name}."
            )
        except Exception as e:
            embed = discord.Embed(
                title="Error!",
            ...
            self.bot.dispatch(
                ...
                ctx.author.id,
                "Deleted a role",
                f"Deleted the role {role.mention}.",
            )
            await ctx.send(embed=embed)
    ...
    async def edit(
        ...
        try:
            ...
            if mentionable is not None:
                await role.edit(
                    mentionable=mentionable, reason=f"Edited by {ctx.author.name}"
                )
            self.bot.dispatch(
                "modlog",
                ctx.guild.id,
