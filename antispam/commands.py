...
from db import antispam_collection, warning_collection
...
class AntiSpam(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if antispam_guild:
            ...
            log_channel = self.bot.get_channel(antispam_guild.get("channel_id", None))
            ...
            if retry_after:
                ...
                if violations.update_rate_limit():
                    ...
                    if log_channel:
                        ...
                        embed.add_field(
                            name="Issued By", value=self.bot.user.mention, inline=False
                        )
                        embed.add_field(
                            name="Issued At",
                            value=f"{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
    ...
    async def anti_spam_enable(self, ctx, channel: discord.TextChannel = None):
        ...
        try:
            if channel is None:
                antispam_collection.insert_one(
                    {"guild_id": ctx.guild.id}, {"$set": {"enabled": True}}
                )
            ...
            self.bot.dispatch(
                ...
                ctx.author.id,
                "Enabled Anti-Spam",
                f"Enabled the Anti-Spam system.",
            )
            await ctx.send(
                "<:switch_on:1326648555414224977> Enabled spam detection.",
                ephemeral=True,
            )
        except Exception as e:
            print(e)
            await ctx.send(
                "An error occurred while enabling spam detection. Please try again later.",
                ephemeral=True,
            )
    ...
    async def anti_spam_disable(self, ctx):
        ...
        try:
            ...
            self.bot.dispatch(
                ...
                ctx.author.id,
                "Disabled Anti-Spam",
                f"Disabled the Anti-Spam system.",
            )
            await ctx.send(
                "<:switch_off:1326648782393180282> Disabled spam detection.",
                ephemeral=True,
            )
        except Exception as e:
            print(e)
            await ctx.send(
                "An error occurred while disabling spam detection. Please try again later.",
                ephemeral=True,
            )
