...
class Notes(commands.Cog):
    ...
    @commands.hybrid_command(
        name="note", description="Leave a note for a user, e.g. known troll."
    )
    ...
    async def note(self, ctx: commands.Context, member: discord.Member, note: str):
        try:
            ...
            embed = discord.Embed(
                title=f"<:Checkmark:1326642406086410317> [#{case_number}] Note left successfully.",
                description="",
    ...
    @commands.hybrid_command(
        name="remove-note", description="Remove a note from a user", aliases=["unnote"]
    )
    @commands.has_permissions(moderate_members=True)
    async def remove_note(
        self, ctx: commands.Context, member: discord.Member, number: int
    ):
        try:
            now = note_collection.count_documents({"guild_id": ctx.guild.id})
            collection = note_collection.find_one(
                {"guild_id": ctx.guild.id, "member_id": member.id, "number": number}
            )
            try:
                self.bot.dispatch(
                    "modlog",
                    ...
                    ctx.author.id,
                    "Removed a Note",
                    f"Removed a note from {member.mention}\nCase Number: {number}",
                )
            except Exception as e:
                print(e)
            try:
                note_collection.delete_one({"guild_id": ctx.guild.id, "number": number})
            except:
                return await ctx.send(
                    "Couldn't find that note, please check the number or the user and try again.",
                    ephemeral=True,
                )
            case_number = note_collection.count_documents({"guild_id": ctx.guild.id})
            embed = discord.Embed(
                title=f"<:Checkmark:1326642406086410317> Note removed successfully, there are now {case_number} notes.",
                description="",
                color=discord.Colour.pink(),
            )
            try:
    ...
    async def list_notes(self, ctx: commands.Context, member: discord.Member = None):
        class notePaginator(View):
            ...
            def get_embed(self):
                ...
                for note in self.notes[start:end]:
                    ...
                    case_number = cases_collection.find_one(
                        {"guild_id": str(ctx.guild.id)}
                    )
                    embed.add_field(
                        name=f"{member.name} {member.id}",
                        value=f'Note: {note["note"]}\nTimestamp: {note["timestamp"]}\nCase Number: {case_number["cases"]}',
                        inline=False,
                    )
                return embed
        ...
        if member is not None:
            notes = [
                note
                for note in note_collection.find(
