...
async def blacklist_check(ctx):
	...
	server_blacklisted = await blacklist_collection.find_one({"_id": ctx.guild.id if ctx.guild else None})
...
async def blacklist(ctx, target: str, target_id: int):
	if target.lower() == "user":
		await blacklist_collection.update_one({"_id": target_id}, {"$set": {"type": "user"}}, upsert=True)
		await ctx.send(f"User `{target_id}` has been blacklisted.")
	elif target.lower() == "server":
		await blacklist_collection.update_one({"_id": target_id}, {"$set": {"type": "server"}}, upsert=True)
