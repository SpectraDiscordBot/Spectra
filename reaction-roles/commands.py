...
class Reaction_Role_Commands(commands.Cog):
    ...
    @commands.hybrid_command(
        name="add-reaction-role", description="Add a reaction role."
    )
    @app_commands.describe(
        role="The role to give when a user reacts",
        label="The label to put on the button",
        button_emoji="The emoji to put on the button",
    )
    async def add_reaction_role(
        ...
        for r in roles:
            ...
            if len(list(roles)) >= 3:
                await ctx.send(
                    "You have reached the maximum limit of 3 reaction roles. Please remove one first."
                )
                return

        roles.append(
            {"label": label, "emoji": str(button_emoji), "role_id": str(role.id)}
        )

        button_roles_collection.update_one(
            {"guild_id": guild_id}, {"$set": {"roles": roles}}, upsert=True
        )
        ...
        self.bot.dispatch(
            ...
            ctx.guild.id,
            ctx.author.id,
            "Added a reaction role",
            f"Added {role.mention} as a reaction role.",
        )

        await ctx.send(
            f"<:Checkmark:1326642406086410317> Added reaction role: `{role.name}` with label `{label}` and emoji {button_emoji}."
        )

    @commands.hybrid_command(
        name="remove-reaction-role", description="Remove a reaction role."
    )
    ...
    async def remove_reaction_role(self, ctx: commands.Context, role: discord.Role):
        ...
        if not data or "roles" not in data:
            await ctx.send(
                "No reaction roles are configured for this guild.", ephemeral=True
            )
            return
        ...
        for idx, role_data in enumerate(roles):
            ...
            await ctx.send(
                f"The role `{role.name}` is not configured as a reaction role.",
                ephemeral=True,
            )
            return

        button_roles_collection.update_one(
            {"guild_id": guild_id}, {"$set": {"roles": roles}}
        )
        ...
        self.bot.dispatch(
            ...
            ctx.guild.id,
            ctx.author.id,
            "Removed a reaction role",
            f"Removed {role.mention} from reaction roles.",
        )

        await ctx.send(
            f"<:Checkmark:1326642406086410317> Removed reaction role: `{role.name}`."
        )

    @commands.hybrid_command(
        name="send-reaction-role", description="Send the reaction roles message"
    )
    ...
    async def send_button_roles(
        ...
        if not roles:
            await ctx.send(
                "No roles have been configured for buttons yet.", ephemeral=True
            )
            return
        ...
        self.bot.dispatch(
            ...
            ctx.guild.id,
            ctx.author.id,
            "Sent the reaction roles",
            f"Sent reaction roles in {channel.mention}.",
        )

        await ctx.send(
            "<:switch_on:1326648555414224977> Button roles setup complete!",
            ephemeral=True,
        )
